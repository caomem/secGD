...
\begin{algorithm}[H]
	...
	\For{$i\in \{K+2,\dots,n\}$}{
		\tcc{Utilize a $(K+1)$-clique dos $(K+1)$ antecessores imediatos de $i$ para calcular a realização $x_i$. Trilateracao() deve retornar $\emptyset$ caso o sistema não tenha solução}
		...
		\For{$\{j\in N(i)\ ; \ j<i\}$}{
			\If{$\lVert x_i - x_j\rVert \ne d_{ij}$}{
				\tcp{Se não, marcar como não factível e sair do loop}
				$x_i = \emptyset$\;
				\textbf{break}\;
			}
		} 
