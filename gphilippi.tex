...
Essa estrutura recursiva dos grafos completos se mantém para o caso geral: Sejam $G_{K+1} = \{V_G, E_G\}$ uma $(K+1)$-clique e $v \in V_G$ um vértice qualquer de $G_{K+1}$, sempre pode-se gerar uma $K$-clique a partir do subgrafo induzido $\langle V_G \setminus \{v\}\rangle$.
...
\subsubsection{Realizações de $(K+1)$-cliques em $\mathbb{R}^{K-1}$ e $\mathbb{R}^{K}$}

Em um primeiro momento, a ideia de necessitar previamente da realização de uma $(K+1)$-clique inicial $G_o$ pode parecer como roubar na solução do problema. Certamente, se o grafo que se está interessado em realizar possuir apenas $K+1$ vértices, essa solução não faz sentido. Por sorte, no caso geral, os problemas de estudo costumam ser bem maiores.

Como o DGP é definido em
