...
%Condição suficiente para um grafo ser globalmente rigido: possuir uma ordenação k-laterativa (edg and aplications, sec 4.1.1).

%\subsubsection{Tamanho da clique inicial}

%Em um primeiro momento, a ideia de necessitar previamente da realização de uma $(K+1)$-clique inicial $G_o$ pode parecer como roubar na solução do problema. Certamente, se o grafo que se está interessado em realizar possuir apenas $K+1$ vértices, essa solução não faz sentido. Felizmente, em geral, os problemas de estudo costumam ser maiores \cite{carlileGDandAplications}. Perceba que, se o problema está imerso no espaço $\mathbb{R}^2$, precisa-se de 3 posições iniciais --- o que, pelo que parece, deu origem ao termo ``triangulação de antenas'', referenciado na cultura pop, normalmente em filmes de Hollywood, onde encontra-se a posição de algo ao utilizar a distância deste a três diferentes antenas.  

%O sistema de posicionamento global (GPS), por exemplo, utiliza da trilateração para descobrir a localização dos aparelhos de GPS (sensores móveis) imersos no espaço $\mathbb{R}^3$. Como, nesse caso, $K =3$, precisa-se de 4 vértices para compor a clique inicial. Esses quatro vértices são satélites com posições fixas (em órbitas geoestacionárias), previamente conhecidas. Note que essas são características que definem um WSNL, onde esses satélites são as ancoras que compõem o problema. Perceba também que lançar e manter um satélite no espaço tem um custo elevado, onde faz sentido perguntar se é possível diminuir essa quantidade de vértices que compõem a clique inicial.
